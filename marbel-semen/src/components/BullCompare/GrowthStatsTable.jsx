...
export default function GrowthStatsTable() {
  ...
  // Random stat generator (for demo)
  const randomStat = (min, max) => {
    const value = (Math.random() * (max - min) + min).toFixed(1);
    const percentile = Math.floor(Math.random() * 40) + 60; // 60–99%
  ...
  const formatStat = (stat) => {
    ...
    return (
      <div className={cls}>
        {sign}
        {stat.value}
        <div className="small">{stat.percentile}%</div>
      </div>
    );
  };
            ...
            <tbody>
              {bulls.map((bull) => (
                <tr key={bull.id}>
                  ...
                  <td>{formatStat(randomStat(-3, 3))}</td>
                  <td>{formatStat(randomStat(0, 10))}</td>
                  <td>{formatStat(randomStat(0, 15))}</td>
                  <td>{formatStat(randomStat(5, 20))}</td>
                  <td>{formatStat(randomStat(-5, 10))}</td>
                  <td>{formatStat(randomStat(-5, 5))}</td>
                  <td>{formatStat(randomStat(400, 600))}</td>
