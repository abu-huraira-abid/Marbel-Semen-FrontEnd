...
export default function CarcassStatsTable() {
  ...
  // random value generator for demo
  const randomStat = (min, max, fixed = 1) => {
    const value = (Math.random() * (max - min) + min).toFixed(fixed);
    const percentile = Math.floor(Math.random() * 40) + 60;
  ...
  const formatStat = (stat) => {
    ...
    return (
      <div className={cls}>
        {sign}
        {stat.value}
        {stat.percentile && <div className="small">{stat.percentile}%</div>}
      </div>
    );
  };
            ...
            <tbody>
              {bulls.map((bull) => (
                <tr key={bull.id}>
                  ...
                  {/* Carcass Stats */}
                  <td>{formatStat(randomStat(10, 25))}</td>
                  <td>{formatStat(randomStat(8, 20))}</td>
                  <td>{formatStat(randomStat(2, 8))}</td>
                  <td>{formatStat(randomStat(-2, 2))}</td>
                  <td>{formatStat(randomStat(3, 6))}</td>
                  <td>{formatStat(randomStat(0.4, 0.9, 2))}</td>
                  <td>{formatStat(randomStat(400, 600, 0))}</td>
